	...
	fs.open(filename, 'w+', safe.sure(cb, function (fd) {
		...
		function get(pos, cb) {
			fs.read(self._fd, b1, 0, 45, pos, safe.sure(cb, function (bytes, data) {
				...
				h1.o = parseInt(h1.o, 10);
				h1.k = parseInt(h1.k, 10);
				var b2 = Buffer.alloc(h1.k + h1.o + 3);
...
tcoll.prototype.createIndex = tcoll.prototype.ensureIndex = function (obj, options, cb) {
	...
	if (key === null) {
		safe.back(cb, new Error("No fields are specified"));
		return cb && cb.promise
	}
...
tcoll.prototype.insertMany = function (docs, opts, cb) {
	return this.insert(docs, opts, cb);
};

tcoll.prototype.insertOne = function (doc, opts, cb) {
		...
		if (_.isObject(v)) {
			switch (v.$wrap) {
				...
				case "$oid":
					...
					break;
				case "$bin":
					var bin = new self._tdb.Binary(Buffer.from(v.v, 'base64'));
					obj[k] = bin;
					break;
...
tcoll.prototype._putFS = function (item, remove, cb) {
	...
	self._wq.add(function (cb) {
		...
		var sobj = Buffer.from(remove ? "" : JSON.stringify(item));
		...
		var skey = Buffer.from(JSON.stringify(key));
		...
		var buf = Buffer.from(JSON.stringify(h1) + "\n" + skey + "\n" + sobj + "\n");
...
tcoll.prototype.deleteOne = function (query, cb) {
	if (!cb) cb = defer()
	this.remove(query, { single: true }, cb);
	return cb && cb.promise
};

tcoll.prototype.deleteMany = function (query, cb) {
	if (!cb) cb = defer()
...
tcoll.prototype.remove = function (query, opts, cb) {
	...
	cb = cb || function () { };
	var single = opts.single || opts.justOne || false;
	this._tq.add(function (cb) {
...
tcoll.prototype.group = _.rest(function (keys, condition, initial, args) {
	...
	var callback = typeof args[args.length - 1] === 'function' ? args.pop() : defer()
	var reduce = args.length ? args.shift() : null,
