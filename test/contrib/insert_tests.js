 ...
 * @ignore
 */
/* REASON: Native stuff is not supported
 ...
 * @ignore
 */
/* REASON: BSON things are not supported
 ...
 * @ignore
 */
/* REASON: TimeStamp is not supported
 ...
 * @ignore
 */
/* REASON: Serialization of functions is not supported
...
/**
 * @ignore
 * TODO: Custom id is fine, but 0 is not
 */
exports.shouldCorrectlyInsertDocWithCustomId = function(configuration, test) {
  ...
  // Insert the update
  collection.insert({_id:1, test:'hello'}, {w:1}, function(err, result) {
    test.equal(null, err);

    collection.findOne({_id:1}, function(err, item) {
      test.equal(1, item._id);
 ...
 * @ignore
 */
/* REASON: Never though it can be used this way
